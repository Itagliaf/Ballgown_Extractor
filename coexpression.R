...
##save the othe data (names etc) and delete them
transcripts<-transcripts[c(0:1000),]

metadata<-transcripts[,c(1:10)]
ok<-transcripts[,-c(1:10)]

##excluding coverage
excude_columns<-seq(1,59,2)
ok<-ok[,-excude_columns]
...
names(datExpr0) = transcripts$gene_name
rownames(datExpr0) = names(ok)
...
net = blockwiseModules(datExpr, power = 7,maxBlockSize=30000,
                       ...
                       saveTOMs = TRUE,
                       saveTOMFileBase = "Complete",
                       verbose = 3)

moduleLabels = net$colors
save(moduleLabels,datExpr,file = "LITTLE_1000_Module_Data.Rdata")
